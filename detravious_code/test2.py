...
class Main(Canvas):
    """The purpose of this class it to create the game canvas, the snake and the obstacle, """
    def __init__(self, boss=None):
        super().__init__(boss)
        self.configure(width=wide, height=tall, bg=BG_COLOR)
        self.running = 0
        self.snake = None
        self.obstacle = None
        self.direction = None
...
class Shape:
    ...
    def __init__(self, can, a, b, kind):
        ...
        self.kind = kind
        if kind == SN:
            self.ref = Canvas.create_rectangle(self.can, a - SN_SIZE, b - SN_SIZE, a + SN_SIZE, b + SN_SIZE,fill=SN_COLOR,width=2)
        elif kind == OB:
            self.ref = Canvas.create_oval(self.can, a - OB_SIZE, b - OB_SIZE, a + SN_SIZE, b + SN_SIZE, fill=OB_COLOR,width=2)
...
class Snake:
    ...
    def __init__(self, can):
        ...
        self.can = can
        a = pixel + 2 * int(overMore/4) * pixel
    ...
    def move(self, path):
        ...
        a = (self.blocks[-1].x + step * path[0]) % wide
        b = (self.blocks[-1].y + step * path[1]) % tall
...
buttons = Frame(root, width=35, height=6*tall/6)
